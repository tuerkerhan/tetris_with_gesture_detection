const video = document.getElementById('video');    
    ...
    .then((stream) => {
        ...
        video.addEventListener('loadeddata', () => detectHands(video));
    });
...
function interpretGesture(landmarks){
    let thumbIsOpen = landmarks[3][0] > landmarks[2][0];
    let indexIsOpen = landmarks[7][0] > landmarks[6][0];
    let middleIsOpen = landmarks[11][0] > landmarks[10][0];

    if(thumbIsOpen && indexIsOpen && !middleIsOpen){
        return 'ROTATE';
...
function performActionBasedOnGesture(gesture){
