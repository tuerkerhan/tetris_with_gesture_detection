...
def landmark_transformation_left(landmark):
    ...
    for point in landmark:
        result.append([1-point[1], point[0]])
    return result
...
def landmark_transformation_right(landmark):
    result = []
    for point in landmark:
        result.append([1-point[1],1-point[0]])
    return result
...
for key, value in tqdm(list(data.items())):
    ...
    if hand == 'right':
        right_hand(f'train_val_like/{key}.jpg', f'train_val_like_left/{key}.jpg')
...
with open('train_val_like_left.json', "w") as file:
    file.write(new_json)
